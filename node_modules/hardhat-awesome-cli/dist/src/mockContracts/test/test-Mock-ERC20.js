...
describe('MockERC20', function () {
    ...
    it('Should return the name of the token', async function () {
        (0, chai_1.expect)(await mockERC20.name()).to.equal('MockERC20');
    });
    it('Should return the symbol of the token', async function () {
        (0, chai_1.expect)(await mockERC20.symbol()).to.equal('MOCK');
    });
    it('Should mint token and have the right balanceOf', async function () {
        const amount = 1000;
        await mockERC20.mint(deployer.address, amount);
        (0, chai_1.expect)(await mockERC20.balanceOf(deployer.address)).to.equal(amount);
    });
    it('Should mint token and have the right totalSupply', async function () {
        const amount = 1000;
        await mockERC20.mint(deployer.address, amount);
        (0, chai_1.expect)(await mockERC20.totalSupply()).to.equal(amount);
    });
    it('Should mint token and burn them', async function () {
        ...
        (0, chai_1.expect)(await mockERC20.balanceOf(deployer.address)).to.equal(0);
    });
    it('Should mint token and transfer them', async function () {
        ...
        (0, chai_1.expect)(await mockERC20.balanceOf(user1.address)).to.equal(1000);
    });
    it('Should mint token and transferFrom them', async function () {
        ...
        (0, chai_1.expect)(await mockERC20.balanceOf(user2.address)).to.equal(1000);
    });
});
//# sourceMappingURL=test-Mock-ERC20.js.map