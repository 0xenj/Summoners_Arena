...
const buildFoundrySetting = async () => {
    if (!fs_1.default.existsSync('foundry.toml')) {
        fs_1.default.writeFileSync('foundry.toml', `[profile.default]
...
block_difficulty = 0                                          # the value of block.difficulty in tests`);
        ...
        console.log('\x1b[33m%s\x1b[0m', 'The Foundry settings already exists at foundry.toml');
    if (!fs_1.default.existsSync('remappings.txt')) {
        fs_1.default.writeFileSync('remappings.txt', `hardhat/=node_modules/hardhat/
@openzeppelin/contracts/=node_modules/@openzeppelin/contracts
@openzeppelin/contracts-upgradeable/=node_modules/@openzeppelin/contracts-upgradeable`);
        console.log('\x1b[32m%s\x1b[0m', 'Creating Foundry settings in remappings.txt');
    }
    else
        console.log('\x1b[33m%s\x1b[0m', 'The Foundry settings already exists at remappings.txt');
    if (fs_1.default.existsSync('contracts')) {
        if (!fs_1.default.existsSync('contracts/test')) {
            fs_1.default.mkdirSync('contracts/test');
            if (!fs_1.default.existsSync('contracts/test/utils')) {
                fs_1.default.mkdirSync('contracts/test/utils');
            }
        }
        else {
            if (!fs_1.default.existsSync('contracts/test/utils')) {
        ...
        fs_1.default.mkdirSync('contracts');
        fs_1.default.mkdirSync('contracts/test');
        fs_1.default.mkdirSync('contracts/test/utils');
    }
    if (require && require.main) {
        ...
        if (fs_1.default.existsSync(packageRootPath)) {
            config_1.DefaultFoundryTestUtilsList.map((testUtils) => {
                if (!fs_1.default.existsSync('contracts/test/' + testUtils)) {
                    fs_1.default.copyFileSync(packageRootPath + '/testForge/' + testUtils, 'contracts/test/' + testUtils);
                    console.log('\x1b[32m%s\x1b[0m', 'Creating Foundry test utilities in contracts/test/' + testUtils);
                }
                else
                    console.log('\x1b[33m%s\x1b[0m', 'The Foundry test utilities already exists in contracts/test/' + testUtils);
            });
        }
    }
};
exports.default = buildFoundrySetting;
//# sourceMappingURL=buildFoundrySetting.js.map